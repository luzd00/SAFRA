...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-200">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-3xl border-2 border-gray-200 shadow-xs">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs sm:text-sm font-black text-gray-900 uppercase tracking-wide">
            O que você quer anotar agora?
          </span>
          <span className="text-xs text-gray-400 font-normal hidden sm:inline">(Clique em um dos botões abaixo)</span>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          <button
            onClick={() => onOpenQuickAction('receita')}
            className="flex items-center justify-center gap-1.5 sm:gap-2.5 px-2.5 sm:px-4 py-3 sm:py-3.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-2xl text-xs sm:text-sm font-extrabold shadow-sm transition-all cursor-pointer text-center"
          >
            <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-200 shrink-0" />
            <span className="truncate sm:whitespace-normal">+ Receita <span className="hidden sm:inline">(Dinheiro)</span></span>
          </button>

          <button
            onClick={() => onOpenQuickAction('despesa')}
            className="flex items-center justify-center gap-1.5 sm:gap-2.5 px-2.5 sm:px-4 py-3 sm:py-3.5 bg-[#0a2e23] hover:bg-[#062118] text-white rounded-2xl text-xs sm:text-sm font-extrabold shadow-sm transition-all cursor-pointer text-center"
          >
            <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400 shrink-0" />
            <span className="truncate sm:whitespace-normal">- Despesa <span className="hidden sm:inline">(Gasto)</span></span>
          </button>

          <button
            onClick={() => onOpenQuickAction('producao')}
            className="flex items-center justify-center gap-1.5 sm:gap-2.5 px-2.5 sm:px-4 py-3 sm:py-3.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-950 border-2 border-emerald-300 rounded-2xl text-xs sm:text-sm font-extrabold transition-all cursor-pointer text-center"
          >
            <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-700 shrink-0" />
            <span className="truncate sm:whitespace-normal">+ Colheita / Leite</span>
          </button>
          ...
          >
            <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-700 shrink-0" />
            <span className="truncate sm:whitespace-normal">+ Veneno / Adubo</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-4 sm:p-6 border-2 border-gray-200 shadow-xs min-w-0 overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-gray-100">
            <div className="flex items-center justify-between sm:block">
              <div>
                <h3 className="text-base sm:text-lg font-black text-gray-900">Entradas e Saídas ao Longo do Ano</h3>
                <p className="text-xs text-gray-500">Comparação dos últimos 12 meses</p>
              </div>
              <span className="sm:hidden text-[10px] text-emerald-800 font-bold bg-emerald-100/70 px-2 py-0.5 rounded-md shrink-0">
                Deslize ↔
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-4 text-xs font-bold flex-wrap">
              ...
              <div className="flex items-center gap-1.5 text-emerald-700">
                <span className="w-3 h-3 rounded-full bg-[#86EFAC] shrink-0" />
                <span>Saiu (Verde claro)</span>
              </div>
            </div>
          </div>

          {/* Bar Chart with Touch Scroll Container */}
          <div className="pt-4 sm:pt-6">
            <div className="overflow-x-auto pb-2 -mx-1 px-1 scrollbar-thin">
              <div className="min-w-[480px] sm:min-w-full">
                <div className="h-60 w-full flex flex-col justify-between">
                  <div className="relative flex-1 flex items-end justify-between gap-1.5 sm:gap-2 px-1 sm:px-2 pb-2">
                    ...
                    {monthlyOverviewData.map((d, index) => {
                      ...
                      return (
                        <div 
                          key={index} 
                          className="relative z-10 flex-1 flex flex-col items-center justify-end h-full cursor-pointer"
                        ...
                        >
                          <div className="w-full flex items-end justify-center gap-1 h-full">
                            <div
                              ...
                              className="w-2.5 sm:w-4 bg-[#0a4835] rounded-t-xs hover:brightness-110 transition-all"
                            />
                            <div
                              style={{ height: `${despHeight}%` }}
                              className="w-2.5 sm:w-4 bg-[#86EFAC] rounded-t-xs hover:brightness-110 transition-all"
                            />
                          </div>
                          <span className="text-[11px] sm:text-xs font-bold text-gray-600 mt-2">
                            {d.month}
                          </span>
                        </div>
                      );
            ...
            <div className="mt-3 p-3 bg-emerald-50 rounded-2xl text-xs text-emerald-900 font-medium">
              {hoveredMonth ? (
                ...
                <span>Toque nas barras ou passe o mouse para ver os valores de cada mês.</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-4 sm:p-6 border-2 border-gray-200 shadow-xs flex flex-col justify-between min-w-0">
          <div>
